...
public class CandidateController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Candidate> findById(@Valid @PathVariable Long id){
        return new ResponseEntity<>(candidateService.findCandidateById(id), HttpStatus.OK);
    }

    @PostMapping("/{id}")
    public ResponseEntity<Candidate> save(@RequestBody Candidate candidate, @PathVariable Long id)throws RecordNotFoundException {
        return new ResponseEntity<>(candidateService.saveCandidate(candidate,id), HttpStatus.OK);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Candidate> update(@RequestBody Candidate candidate, @PathVariable Long id){
